    ...
    */
    ...
    function initStorySlider() {
        ...
        function goToSlide(targetIndex) {
            ...
            // Create timeline for sequential fade transition (no overlap)
            const tl = gsap.timeline({
                onComplete: () => {
                    console.log('Transition complete, updating navigation');
                    // Update navigation after transition
                    updateNavigation(targetIndex, slides.length, navContainer);
                    currentSlideIndex = targetIndex;
                }
            });

            // Step 1: Fade out current slide completely
            tl.to(currentSlide, {
                opacity: 0,
                duration: 0.25,
                ease: "power2.inOut"
            })
                // Step 2: Hide current slide and prepare target slide (happens instantly)
                .set(currentSlide, {
                    visibility: 'hidden',
                    position: 'absolute',
                    top: 0,
                ...
                // Step 3: Fade in target slide (starts after previous slide is completely hidden)
                .to(targetSlide, {
                    opacity: 1,
                    duration: 0.25,
                    ease: "power2.inOut"
