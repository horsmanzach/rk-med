			...
			<article id="post-<?php the_ID(); ?>" <?php post_class( 'et_pb_post clearfix' . $no_thumb_class . $overlay_class . $item_class ); ?>>
				...
				<?php } ?>
                    ...
                    // =========Add Custom Taxonomy Terms
                       ...
                       // Get the treatment terms for this post
                   $treatments = get_the_terms($post->ID, 'treatment');

                 if ($treatments && !is_wp_error($treatments)) {
                $first_treatment = reset($treatments);
                echo '<div class="treatment-category">' . esc_html($first_treatment->name) . '</div>';
                    }       


                    //========Adding Custom Fields

                    $quote = get_post_meta($post->ID, 'patient_quote', true);
						if ($quote != '') {
    					 echo '<div class="patient-quote">' . esc_html($quote) . '</div>';
					} 
